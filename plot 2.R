...
par(mfrow = c(1,1))
par(mar = c(4, 5, 1, 4))
plot(hh_power_consumption$Global_active_power ~ hh_power_consumption$timestamp, type = "l", 
     xlab="",ylab="Global Active Power (killowatts)")

dev.copy(png, file = "./ExData_Plotting1/plot2.png", width = 480, height = 480)
dev.off()

with(hh_power_consumption, plot(Global_active_power ~ timestamp, type = "l"))